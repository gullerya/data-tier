(() => {
	...
	suite.addTest({name: 'testing custom elements - autonomous: undefined first and adding to DOM'}, (pass, fail) => {
		...
		e.dataset.tieValue = 'testCustomsUndefined.text';
		...
		setTimeout(() => {
			...
			setTimeout(() => {
				if (e.value !== tie.data.text.toUpperCase()) fail('textContent of the element expected to be ' + tie.data.text.toUpperCase() + ', found: ' + e.value);
				pass();
			}, 0)
		}, 0);
	});
	...
	suite.addTest({name: 'testing custom elements - extended: undefined first and adding to DOM'}, (pass, fail) => {
		...
		e.dataset.tieProperty = 'testCustomsUndefined.text => valueA';
		if (e.value) fail('precondition failed - was not expecting to get any value, got "' + e.value + '"');
		...
		setTimeout(() => {
			...
			setTimeout(() => {
				if (e.value !== tie.data.text.toUpperCase()) fail('value of the element expected to be ' + tie.data.text.toUpperCase() + ', found: ' + e.value);
				pass();
			}, 0)
		}, 0);
	});
